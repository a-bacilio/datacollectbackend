...
const deleteQuestionService = async (body: { projectId: string, questionId: string}): Promise<{message:string}> => {
  try {
    ...
    const questions = project?.questions?.filter(question=>question.toString() !== body.questionId);
