...
const questionDownByIdService = async (body: { projectId: string, order: number }): Promise<IProject | null> => {
  try {
    ...
    if (body.order>=0 && body.order<=questions.length-2 && questions.length>=2) {
